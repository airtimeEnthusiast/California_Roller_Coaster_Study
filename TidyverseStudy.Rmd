...
|B. | Brief History and Notable Trends                      |
...
```{r Existant Coasters statisical breakdowns,echo=FALSE}
...
```{r Population Perctange,echo=FALSE}
...
Santa Cruz's Giant Dipper features more of an out and back style layout with airtime hills. It is my favorite of the two especially this innocent looking  drop I call ["The Ejector death hill"](https://youtu.be/ZmZvKYtc7g0?t=130)
...
## 2A. Before 1980
```{r Before 1980 Stats, ehco=FALSE}
...
```{r Focus on Coasters After 1980,, ehco=FALSE}
...
```{r Inversion historgram, ehco=FALSE}
...
```{r height historgram over 200, ehco=FALSE}
...
```{r Avg Height for each Manufacture, ehco=FALSE}
...
```{r Avg Speed for each Manufacture, ehco=FALSE}
...
```{r Popular Models within eras, ehco=FALSE}
...
```{r Parks with the most coasters, ehco=FALSE}
