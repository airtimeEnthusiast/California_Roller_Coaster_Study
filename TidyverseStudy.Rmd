...
```{r Height above 50ft VS Speed}
filteredDF <- subset(currentDataFrame, subset=((Height >= 100)))
scatterPlot <- filteredDF %>% 
  ggplot(mapping = aes(x = Speed, y = Height, 
             text = paste(
             Name, " at ", Park, "\n", 
                "Description: ", Type ,"", Model.Name, "\n",
  ...
  labs(x = "Speed", 
       y = "Height",
       title = "Speed compared to Height Of All Coasters Over 50ft") +
  geom_hline(yintercept=0, linetype="dashed", color = "#2C528C", size=0.5) + geom_vline(xintercept=0, linetype="dashed", color = "#2C528C", size=0.5) 
...
#text <- c(currentDataFrame$Name)
data2 <- data.frame(c_year, c_height, text)
...
## 4 Above 200ft in Coaster Histogram
